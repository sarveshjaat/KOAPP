...
import com.nictko.services.databinding.ActivityLimitReportViewBinding
import com.nictko.services.sharedprefrence.SessionManager
import com.nictko.services.viewmodel.LimitReportViewModel
import java.text.SimpleDateFormat
import java.util.Calendar
import java.util.Date

class LimitReportViewActivity : AppCompatActivity() {
    private lateinit var binding: ActivityLimitReportViewBinding
    private val viewModel by viewModels<LimitReportViewModel>()
    private lateinit var adapter: LimitReportAdapter
    private lateinit var progressDialog: Dialog

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_limit_report_view)
...
//                                    binding.tvBusddate.setText(dayOfMonth + "/" + (monthOfYear + 1) + "/" + year);
        ...
        // Initialize Adapter
        adapter = LimitReportAdapter(emptyList())

        binding.recyclerview.adapter = adapter
        viewModel.limitreportResult.observe(this) {
...
//            sendreqaddsettlelmnt()
    ...
    fun sendreqaddsettlelmnt() {
        ...
        val setpage = 1
        val setsize = 10
        viewModel.calllimitreport(getstartdate=setstartdate, gettodate=settodate,getkoid = setkoid!!,getpage= setpage ,getsize= setsize)
    }
